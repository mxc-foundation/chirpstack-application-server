.PHONY: build clean test ui-requirements serve pkg/statics pkg/migrations
...
build: pkg/statics pkg/migrations
	mkdir -p build
...
clean:
	...
	@rm -rf build dist pkg/migrations/migrations_gen.go pkg/static/static_gen.go ui/build static/static
...
test: pkg/statics pkg/migrations
	@echo "Running tests"
	@rm -f coverage.out
	@for pkg in $(PKGS) ; do \
		golint $$pkg ; \
...
dist: ui/build pkg/statics pkg/migrations
	@goreleaser
...
snapshot: ui/build pkg/statics pkg/migrations
	@goreleaser --snapshot
...
pkg/statics pkg/migrations: static/swagger/api.swagger.json
...
static/swagger/api.swagger.json:
	...
	@GOOS="" GOARCH="" go run pkg/tools/swagger/main.go /tmp/chirpstack-api/swagger/as/external/api > static/swagger/api.swagger.json
